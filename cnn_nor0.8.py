...
train_images = np.load("../en5_en6_19s_noshift_bin2/train_x_en5_en6_19s_300_16000_noshift_bin2.npy")
train_labels = np.load("../en5_en6_19s_noshift_bin2/train_y_en5_en6_19s_300_16000_noshift_bin2.npy")
test_images = np.load("../en5_en6_19s_noshift_bin2/test_x_en5_en6_19s_300_2522_noshift_bin2.npy")
test_labels = np.load("../en5_en6_19s_noshift_bin2/test_y_en5_en6_19s_300_2522_noshift_bin2.npy")
...
## conv1 layer ##
W_conv1 = weight_variable([5,5,1,16]) #patch 4x4, in size 1, out size 16
b_conv1 = bias_variable([16])
h_conv1 = tf.nn.tanh(conv2d(x_images, W_conv1) + b_conv1) #output size 200x200x16
h_pool1 = max_pool_2x2(h_conv1)    #output size 100x100x16



## conv2 layer ##
W_conv2 = weight_variable([5,5,16,32]) #patch 4x4, in size 16, out size 32
b_conv2 = bias_variable([32])
h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2) + b_conv2) #output size 100x100x32
h_pool2 = max_pool_2x2(h_conv2)  #output size 34x34x32


## conv3 layer ##
W_conv3 = weight_variable([5,5,32,64]) #patch 4x4, in size 32, out size 64
...
## fc1 layer ##
W_fc1 = weight_variable([12*12*64, 512])
...
#cross_entropy = tf.reduce_mean(-tf.reduce_sum(ys*tf.log(prediction + 1e-8))) + 0.8*(tf.reduce_mean(tf.reduce_sum(W_fc3[:]**2)) + tf.reduce_mean(tf.reduce_sum(b_fc3[:]**2)))/2
cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=ys), name="cross_entropy")
#cross_entropy = tf.reduce_mean(cross_en, name = "cross_entropy")
...
#saver.restore(sess, "./saver_nor0.8/cnn_nor.ckpt")

 doc=open('./out/out_nor0.8.txt','w')
...
#    train_labels_rand = train_labels[permutation,:]
    ...
    save_path = saver.save(sess, "./saver_nor0.8/cnn_nor.ckpt", global_step=i)
